...
from ncaa_mbb_data_build.config import PKG_FUNCTION, REGISTRY
...
def test_stamp_uploads_the_four_sidecars_with_clobber():
    ...
    publish._stamp(
        "ncaa_mbb_lineups", calls.append, "sportsdataverse/sportsdataverse-data"
    )

    assert [Path(c[3]).name for c in calls] == SIDECAR_NAMES
    assert all(c[:3] == ["release", "upload", "ncaa_mbb_lineups"] for c in calls)
...
def test_stamp_names_the_loader_for_the_tag():
    ...
    publish._stamp("ncaa_mbb_lineups", _runner, "sportsdataverse/sportsdataverse-data")

    expected = PKG_FUNCTION["ncaa_mbb_lineups"]
