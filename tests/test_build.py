...
def test_build_season_pbp_direct(tmp_path: Path):
    ...
    pq = tmp_path / "mbb" / "pbp" / "parquet" / "ncaa_mbb_pbp_2026.parquet"
    assert pq.exists()
...
def test_build_season_schedule_derived(tmp_path: Path):
    ...
    assert out.height == 8
    pq = tmp_path / "mbb" / "schedule" / "parquet" / "ncaa_mbb_schedule_2026.parquet"
    assert pq.exists()
...
def test_build_season_team_ids_derived(tmp_path: Path):
    ...
    pq = tmp_path / "mbb" / "team_ids" / "parquet" / "ncaa_mbb_team_ids_2026.parquet"
    assert pq.exists()
