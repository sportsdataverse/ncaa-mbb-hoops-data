...
committed tree's own layout, so nothing here depends on the built ``mbb/``
...
from ncaa_mbb_data_build.master import (
    GAME_LEVEL,
    add_game_detail,
    build_coverage,
    build_master,
...
def _write_dataset(base: Path, key: str, season: int, frame: pl.DataFrame) -> Path:
    spec = REGISTRY[key]
    d = base / "mbb" / spec.dataset / "parquet"
...
def _write_raw_master(root: Path, rows: dict[str, list]) -> None:
    (root / "mbb").mkdir(parents=True, exist_ok=True)
    pl.DataFrame(rows).write_parquet(root / "mbb" / "mbb_schedule_master.parquet")
