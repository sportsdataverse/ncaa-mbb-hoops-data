...
``ncaa_mbb_data_build.build``; this file exists so the stage sequence is readable from a
directory listing.

Stage numbers follow the ``REGISTRY`` order in ``ncaa_mbb_data_build/config.py``, which is
...
Equivalent to::
    ...
    python -m ncaa_mbb_data_build build --dataset matchup_stints --season <year>
"""
