...
def _key_name(name: str) -> str:
    ...
    drop suffix tokens, merge single-letter tokens (initials), sort.
    Both forms above map to ``"derrick talton"`` / ``"bj edwards"``.
    """
    import unicodedata
...
def team_rosters(season: int, raw_root: "str | Path") -> pl.DataFrame:
    ...
    Reads the per-team JSONs the raw repo's ``ncaa_mbb_04_rosters_scrape.py`` stage captures
    ...
    base = Path(raw_root) / "mbb" / "team_rosters" / str(season)
    rows: "list[dict]" = []
    if base.is_dir():
