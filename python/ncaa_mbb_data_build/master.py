...
- Every ``in_*`` flag is stamped from the COMMITTED per-season parquet of that
  dataset (``mbb/{dataset}/parquet/{stem}_{season}.parquet``) -- the exact
...
from ncaa_mbb_data_build import ingest
from ncaa_mbb_data_build.config import REGISTRY, DatasetSpec
...
def denominator(*, raw_root: str | Path | None = None) -> pl.DataFrame:
    ...
    if raw is None:
        raise FileNotFoundError(
            "no schedule master in the raw checkout; set NCAA_MBB_RAW_ROOT to the "
            "ncaa-mbb-hoops-raw root (or pass raw_root=)"
        )
