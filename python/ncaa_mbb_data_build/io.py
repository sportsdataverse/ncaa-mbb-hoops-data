...
``mbb/{dataset}/parquet/{stem}_{season}.parquet``. Release assets (csv.gz here,
...
def write_dataset(
    ...
    *,
    ...
    ``{base}/mbb/_release_build/{dataset}/{stem}_{season}.csv.gz``. Upserts the
    ...
    **Why gzip, not plain csv:** one season of ``pbp`` is ~3.1M rows and writes
    ...
    (~95 MiB), 21.3x smaller, and the cliff is gone. Byte counts rather than
    rounded GB/MB, because the thing being cleared is a hard limit measured in
    bytes. ``espn_cfb_model_pbp`` already ships ``.csv.gz`` on this same release
    repo, so the extension is not novel for consumers.
    """
    base = Path(base)
