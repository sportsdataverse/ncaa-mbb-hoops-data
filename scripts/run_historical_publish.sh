#!/usr/bin/env bash
# Historical backfill: build + publish EVERY season of EVERY ncaa_mbb_* dataset.
#
...
[ -z "$DATASETS" ] && DATASETS="$(uv run python -c 'from ncaa_mbb_data_build.config import REGISTRY; print(" ".join(REGISTRY))')"
...
say "raw root: ${NCAA_MBB_RAW_ROOT}"
...
for (( season=START; season>=END; season-- )); do
  ...
  for ds in $DATASETS; do
    pq="mbb/${ds}/parquet/ncaa_mbb_${ds}_${season}.parquet"
    ...
    if [ "$FORCE" != "1" ] && [ -s "$pq" ] && \
       awk -F, -v d="$ds" -v s="$season" 'NR>1 && $1==d && $2==s {found=1} END {exit !found}' \
           "mbb/${ds}/manifest.csv" 2>/dev/null; then
      skip=$((skip+1)); continue
    fi
    t0=$SECONDS
    if uv run python -m ncaa_mbb_data_build build \
