...
2. **The published frame is filtered to season *t*.** `season_slice` (sdv-py) inner-joins
   ...
   `tests/test_rapm_leakage_boundary.py` proves both halves: a whole `run_season` on a
   real season is instrumented and no *possession or box-score* input for a season after
   the target is read, and a pooled fit that rates a prior-season-only player publishes a
   frame without him, on the target season's possessions rather than the two-season sum.
   The cross-season identity bridge is the one deliberate exception — it is built once
   over every roster season 2010–2026, so a `person_id` merge can be informed by a later
   roster; that channel carries identity only, never performance data, and narrowing it to
   seasons ≤ *t* is recorded as open rather than claimed.
